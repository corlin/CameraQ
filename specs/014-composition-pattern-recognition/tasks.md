...
- T047: 100 deterministic degraded cases and 50 paired recommendation cases are accepted and hash-audited. The initial 439 unique licensed Wikimedia candidates were reviewed with the user-authorized temporary Codex visual pass: 252 accepted, 169 rejected, and 18 retained pending; the reviewer identity explicitly records Codex rather than representing the result as human ground truth. Targeted Commons collection then added 22 `Horizons` and 18 `Skyscrapers` candidates; 20 and 18 respectively passed visual review, bringing the manifest to 479 Commons candidates and 440 accepted cases total. Horizontal and vertical positives now clear the 20-case gate. The full prelabel and high-confidence decisions are retained as CSV audit artifacts, imported into the manifest, and reflected in the regenerated queue/offline page. Regression tests ensure accepted multi-label cases count `labels` and cross-mode `negative_for` independently and that newly downloaded, not-yet-viewed contact-sheet indices default to `pending` rather than rejection. There are 26 remaining count issues, so T047 remains open. Next work is targeted licensed candidate collection for the recorded negative and remaining positive deficits, original-image spot checks, and accepted calibration/acceptance split assignment.
- T048: degraded abstention and recommendation improvement/action metrics are recorded at 100%. Runtime thresholds and all 15 scorers' evidence weights now have complete centralized maps; current rule behavior is versioned as `initial-rule-weights-v1`. The calibration tool embeds that weight snapshot, enforces positive/negative examples in disjoint calibration/acceptance splits, selects a precision-constrained enter threshold, preserves a hysteresis gap, and reports holdout false negatives. Real-image precision/recall, Top 3 calibration, and final calibrated weights/thresholds remain pending reviewed data.
